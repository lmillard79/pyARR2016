...
fout.write('Event,IL,CL,Alpha,Beta,m,Location,Cal_QP,Rated_QP,Peak Ratio,Cal_Vol,Rated_Vol,Volume Ratio,Cal_HP,Rec_HP,delta H,Nash-Sutcliffe\n')
...
for idx,f_lines in enumerate(f2):
    if idx ==0:
        ...
        pqhpath = pqhpath_prefix + '\\' + f_lines[0:-1] + '\\' + pqhfolder + '\\' + pqfile + '.pqh'
        ...
        for p_lines in p2:
            ...
            if count > header:
                ...
                fout.write('%s,%s,%s,%s,%s,%s,' %(f_lines[0:-1],IL,CL,alpha,beta,m))
                try:
                    rec_Q = textlist[10]
                    rec_V = textlist[11]
                    rec_H = textlist[12]
                ...
                fout.write('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' %(location,cal_Q,rec_Q,PR,cal_V,rec_V,VR,cal_H,rec_H,dH,NS))
