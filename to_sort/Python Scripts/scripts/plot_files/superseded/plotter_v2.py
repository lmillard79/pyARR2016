def plot_urbs(pqhpath,event,catchment,csv_results):
    ...
    for values in time_series_raw:
        time_series.append(datetime(1899,12,30) + timedelta(days=values))
    ...
    while xx <= max(field_count-1,1):
        ...
        if test2[0:-4] == test3[0:-4] and test1[0:-4] != test2[0:-4]:
            ...
            #if l_rain_t != 'False':
            #    newax=ax.twinx()
            #    B1 = newax.bar(x,r1, width, color = "cyan", label=rain_header_list[yy],edgecolor = 'none')
            #    B2 = newax.bar(x,r2, width, color = "grey", label=e_rain_header_list[yy])
            ...
            #    newax.set_ylabel ('Rainfall (mm/hr)',fontsize = 10)
            ...
            ax.xaxis.set_major_locator(mdates.HourLocator(9,24))
